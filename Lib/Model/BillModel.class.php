...
class BillModel extends MongoModel{
	...
	*/
	public function selectBySiteIdWithPage($seatArr,$page=1,$limit=10,$field = null,$time = null){
		...
		//根据时间，筛选出不同状态的广告(广告投放)
		if($time){
			// $arr['where']['put.time']  = array('between', array(new MongoDate(1358380800),new MongoDate(1358899200)));
		}
		// print_r($arr);
...
// 		}
// 		if($time){
// 			// $arr['where']['put.time']  = array('lt', new MongoDate(1356883200));
// 			$arr['where']['put.time']  = array('gt', new MongoDate(1355932800));
// 		}
// 		// print_r($arr);
// 		$rs = $this->order('_id desc')->select($arr);
// 		// print_r($rs);
